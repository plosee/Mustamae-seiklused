...
@export (Array, int) var repeat_levels = [1]: set = set_repeat_levels
@export (Array, Vector2) var repeat_offsets = [Vector2(100, 0)]: set = set_repeat_offsets
...
func set_repeat_levels(value):
	...
	while len(repeat_levels) > len(repeat_offsets):
		repeat_offsets.append(Vector2(100, 0))
